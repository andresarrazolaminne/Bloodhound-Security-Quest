...
- July 17, 2025. Implemented progress text color customization system - added progressTextColor field to database schema and system configuration, created admin panel control in "Personalización" → "Colores" tab alongside other text color controls, updated ProgressBar component to accept configurable color prop, and fixed Zod schema validation to ensure proper saving of progress text color changes
- July 27, 2025. Fixed critical modal display bug for already-scanned segments - resolved issue where modals with custom content (like YouTube iframes) weren't showing when users re-scanned QR codes through the in-app scanner. Problem was in MapPage.tsx alreadyScanned handler which didn't check for modalContent. Added proper modal verification and mobile-responsive CSS optimizations for iframe content. Both external QR URLs (QRUnlockHandler) and internal scanner (MapPage) now correctly display modals for previously unlocked segments